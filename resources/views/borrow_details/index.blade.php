                ...
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-semibold">
                            {{ $borrowDetails->where('borrowRequest.status', 'approved')->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-semibold">
                            {{ $borrowDetails->where('borrowRequest.status', 'pending')->count() }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div
                    ...
                    <div class="relative">
                        @if ($detail->itemUnit->item->image_url)
                            <img src="{{ $detail->itemUnit->item->image_url }}" alt="{{ $detail->itemUnit->item->name }}"
                        ...
                        {{ $detail->borrowRequest->status === 'approved'
                            ? 'bg-green-100 text-green-800'
                            : ($detail->borrowRequest->status === 'pending'
                    ...
                    <div class="p-4">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">SKU:
                                {{ $detail->itemUnit->sku }}</span>
                        </div>
                        ...
                        <div class="flex items-center text-sm text-gray-500 mb-3">
                            ...
                            Kembali:
                            {{ \Carbon\Carbon::parse($detail->borrowRequest->return_date_expected)->format('d M Y') }}
                        </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if ($detail->itemUnit->condition === 'baik')
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-green-500 ml-1"
                                    ...
                                    @elseif($detail->itemUnit->condition === 'rusak')
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-red-500 ml-1"
                                            viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd"
