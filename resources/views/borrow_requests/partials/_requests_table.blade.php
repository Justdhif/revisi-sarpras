            ...
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col"
                        class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a
                            href="{{ route('borrow-requests.index', array_merge(request()->query(), ['sort' => 'return_date_expected', 'direction' => $sortField === 'return_date_expected' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}">
                            Tanggal Pengembalian
                            @if ($sortField === 'return_date_expected')
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <img class="h-10 w-10 rounded-full" src="{{ $request->user->profile_picture }}"
                                        alt="{{ $request->user->name }}">
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $request->user->name }}</div>
                                    <div class="text-sm text-gray-500">{{ $request->created_at->diffForHumans() }}
                                    </div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                {{ Carbon\Carbon::parse($request->return_date_expected)->format('d M Y H:i') }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                ...
                                @if ($request->status === 'pending')
                                    ...
                                    <!-- Form Reject -->
                                    <form action="{{ route('borrow-requests.reject', $request->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                ...
                                @else
                                    <a href="{{ route('borrow-requests.edit', $request->id) }}"
                                    ...
                                    <form action="{{ route('borrow-requests.destroy', $request->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                ...
                                @if (
                                    $request->status === 'approved' &&
                                        \Carbon\Carbon::parse($request->return_date_expected)->isPast() &&
                                        (!$request->returnRequest || $request->returnRequest->status != 'approved'))
                                    <form action="{{ route('borrow-requests.sendReminder', $request->id) }}"
                                        method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="text-amber-600 hover:text-amber-900 bg-amber-50 hover:bg-amber-100 px-3 py-1.5 rounded-md transition-colors duration-200 flex items-center">
                                            ...
                                            <span class="ml-1">Kirim Pengingat</span>
                                        </button>
                                    </form>
                                @endif
                            </div>
    ...
    @if ($borrowRequests->total() > 0)
        <div class="mt-6 bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 rounded-b-lg">
            <div class="flex-1 flex justify-between sm:hidden">
                ...
                @else
                    <a href="{{ $borrowRequests->previousPageUrl() }}"
                ...
                @if ($borrowRequests->hasMorePages())
                    <a href="{{ $borrowRequests->nextPageUrl() }}"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Showing
                        <span class="font-medium">{{ $borrowRequests->firstItem() }}</span>
                        to
                        <span class="font-medium">{{ $borrowRequests->lastItem() }}</span>
                        of
                        <span class="font-medium">{{ $borrowRequests->total() }}</span>
                        results
                    </p>
                </div>
                <div>
