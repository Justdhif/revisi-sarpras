<div x-show="requests.length > 0" class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900" x-text="request.requester.name">
                                    </div>
                                    <div class="text-sm text-gray-500"
                                        x-text="new Date(request.created_at).toLocaleDateString()"></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900"
                                x-text="request.return_date ? new Date(request.return_date).toLocaleDateString() : '-'">
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                x-bind:class="{
                                    ...
                                    'bg-blue-100 text-blue-800': request.status === 'returned',
                                    'bg-gray-100 text-gray-800': !['pending', 'approved', 'rejected', 'returned']
                                        .includes(request.status)
                                }"
                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full">
                                <span
                                    x-text="request.status === 'pending' ? 'Menunggu' :
                                             request.status === 'approved' ? 'Disetujui' :
                                             request.status === 'rejected' ? 'Ditolak' :
                                             request.status === 'returned' ? 'Dikembalikan' : request.status"></span>
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-600" x-text="request.approver ? request.approver.name : '-'">
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                <a x-bind:href="'{{ route('borrow-requests.index') }}/' + request.id"
                                ...
                                <template x-if="request.status === 'pending'">
                                    <a x-bind:href="'{{ route('borrow-requests.index') }}/' + request.id + '/edit'"
                                ...
                                <template x-if="request.status === 'pending'">
                                    <form x-bind:action="'{{ route('borrow-requests.index') }}/' + request.id"
