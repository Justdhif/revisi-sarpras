...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-6 bg-white p-4 rounded-lg shadow border border-gray-100">
            <form action="{{ route('item-units.index') }}" method="GET">
                <div class="flex items-center justify-center gap-4">
                    ...
                    <div class="w-2/3">
                        <select name="condition"
                            ...
                            <option value="refurbished" {{ request('condition') === 'refurbished' ? 'selected' : '' }}>
                                Rekondisi</option>
                            <option value="damaged" {{ request('condition') === 'damaged' ? 'selected' : '' }}>Rusak
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="w-2/3">
                        <select name="status"
                            ...
                            <option value="reserved" {{ request('status') === 'reserved' ? 'selected' : '' }}>Dipesan
                            </option>
                            <option value="out_of_stock" {{ request('status') === 'out_of_stock' ? 'selected' : '' }}>Habis
                            </option>
                        </select>
                    </div>
                        ...
                        <select name="warehouse"
                            ...
                            @foreach ($warehouses as $warehouse)
                                <option value="{{ $warehouse->id }}"
                                    {{ request('warehouse') == $warehouse->id ? 'selected' : '' }}>{{ $warehouse->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('unitTable', () => ({
                ...
                submitSearchForm(searchTerm) {
                    ...
                    form.method = 'GET';
                    form.action = '{{ route('item-units.index') }}';
                    ...
                    if (conditionSelect.value) {
                        urlParams.set('condition', conditionSelect.value);
                    } else {
                    ...
                    if (warehouseSelect.value) {
                        urlParams.set('warehouse', warehouseSelect.value);
                    } else {
